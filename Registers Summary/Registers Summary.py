...
for a in file.readlines():            # przejcie linijka po linijce przez plik
    ...
    numery_przed.append(a[1])         # rozpakowanie danych z listy 'a' i wpisanie do odpowiednich list z linijek 19-22
...
for i in range(len(unique)):
    ...
    jedns.append(jedn_ok)                                                # wypełnienie list (z linijek 49-52) zbierających dane ostateczne
    pows.append(pow_ok) 
    pkts.append(pkt_ok)
    nrs.append(nr_ok)
...
data_przed = pd.DataFrame(data = [jedns, nrs, pows, pkts]).transpose()
data_przed.columns = ['jedn_przed','nr_przed','pow_przed','pkt_przed']
...
for a in file.readlines():         # przejcie linijka po linijce przez plik
    ...
    numery_po.append(a[1])         # rozpakowanie danych z listy 'a' i wpisanie do odpowiednich list z linijek 85-88
...
for i in range(len(unique)):
    ...
    jedns.append(jedn_ok)                                             # wypełnienie list (z linijek 111-114) zbierających dane ostateczne
    pows.append(pow_ok) 
    pkts.append(pkt_ok)
    nrs.append(nr_ok)
...
data_po = pd.DataFrame(data = [jedns, nrs, pows, pkts]).transpose()
data_po.columns = ['jedn_po','nr_po','pow_po','pkt_po']
...
for i in range(1, max_jedn + 1):                                           # przejcie przez wszystkie numery jednostek (i) - od 1 do max_jedn
    ...
    po_roboczy = po_roboczy.sort_values(by = 'nr_po', ignore_index = True)                       # posortowanie nr działek oraz reset indeksu 
    ...
    joined.fillna(value = int(0), inplace = True)                          # wypełnienie wartosci NaN zerami (wartosc numeryczna i zawsze się zsumuje z resztą)
    ...
    if len(joined) == 0:                # jesli pusty DataFrame (nie było takiego nr jedn. rej. przed i po scaleniu - to nie rób nic i idź dalej)
        ...
        lp += 1                                                      # inkrementacja liczby porządkowej Lp po skończonym zapisie danej jednostki
